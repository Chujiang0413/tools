...
public class WordUtils {
    private List<String> ispercentArr;

    public static void main(String[] args) throws Exception {
        boolean result=false;
        List<HashMap> dataList=new ArrayList<HashMap>();//数据
        List<String> fldNameArr = new ArrayList<String>();// 字段名
        List<String> titleArr = new ArrayList<String>();// 标题
        HashMap baseFormMap=new HashMap();
        ...
        HashMap baseFormMap1=new HashMap();
        ...
        HashMap baseFormMap2=new HashMap();
        ...
        HashMap baseFormMap3=new HashMap();
        ...
        HashMap baseFormMap4=new HashMap();
        ...
        HashMap baseFormMap5=new HashMap();
...
//        result=createChart(wb,sheet,10,"bar",STBarGrouping.STACKED,false,false,dataList, fldNameArr, titleArr,showtailArr,ispercentArr);
        ...
        result=createChart(wb,sheet,10+dataList.size()+12,"bar",STBarGrouping.PERCENT_STACKED,true,true,dataList, fldNameArr, titleArr,showtailArr,ispercentArr);
        ...
        FileOutputStream fileOut = new FileOutputStream("D://BarChart3.xlsx");
        wb.write(fileOut);
        fileOut.close();
    }
    ...
    public static boolean refreshChartExcel(SXSSFWorkbook wb,SXSSFSheet sheet,
                                            List<HashMap> dataList,List<String> fldNameArr,List<String> titleArr,
        ...
        for (int i = 0; i < dataList.size(); i++) {
            HashMap baseFormMap = dataList.get(i);//数据行
            //fldNameArr字段属性
            for (int j = 0; j < fldNameArr.size(); j++) {
                if(sheet.getRow(position+i+1)==null){
                    if(j==0){
                        try {
                            Object object = baseFormMap.get(fldNameArr.get(j));
                            String objectStr = "";
                            if (object == null) {
                                objectStr = "";
                            } else {
                            ...
                            sheet.createRow(position+i+1).createCell(j).setCellValue(objectStr);
                        } catch (Exception e) {
...
//                            }
                    ...
                    if(value==0){
                        ...
                        sheet.getRow(position+i+1).createCell(j).setCellValue(b.doubleValue());
                    }
     ...
     * @return
     */
    public static boolean createChart(SXSSFWorkbook wb,SXSSFSheet sheet, int curRow,String type, Enum group,boolean isLegend,boolean isvalAxis, List<HashMap> dataList, List<String> fldNameArr, List<String> titleArr,List<String> showtailArr,List<String> ispercentArr) {
        boolean result=false;
